...
dirs = directory_url + '/' + str(new_dir) + '/' + (u'首页推荐')
...
for i in range(1, 5):
    if (i == 1):
        url = "http://trb226.com/index.html"
    else:
        url = "http://trb226.com/page/" + str(i) + ".html"

    # 解析网页
    webdata = requests.get(url)
    html = BeautifulSoup(webdata.content, 'lxml')
    # 获取 当前页面所有文章的a链接
    a_list = html.select(".excerpt-one h2 a")
    # 将a链接保存
    a_list_all += a_list
...
for html_a in a_list_all:
    ...
    url = "http://trb226.com" + str(html_a["href"])
    # 解析网页
    webdata = requests.get(url)
    html = BeautifulSoup(webdata.content, 'lxml')
    img_htmls = html.select(".article-content p img")
    i =  1
